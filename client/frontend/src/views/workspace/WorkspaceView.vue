				...
				<div class="w-full workspace-content-box-item layout-side relative" v-else>
					...
					<div class="right layout-left-top">
						...
						<template v-if="w.status === 0">
							<template v-if="w.ipType[0] === '普通宽带' && w.ipType[1] === '家庭宽带' && w.ipType[2] === '城域网'">
								<div class="w-full h-1/3 layout-center cursor-pointer"
						...
						<div v-else @click="onRun(w,index)" class="w-full h-1/3 layout-center cursor-pointer"
								 :class="[(w.status === 1 || w.status === 2)?'horse_run':'']"
								 style="color: #ffffff;border-radius: 0 0 6px 6px;"
								 :style="{backgroundColor: w.status === 1 ? '#FF9800' : w.status === 2 ? '#4CAF50' : '#9E9E9E'}">
							{{ w.status === 1 ? '任务启动中' : w.status === 2 ? '任务执行中' : '任务已执行'
							}}
						</div>
					</div>
...
<script setup>
	...
	onMounted(async () => {
		...
		browserData.value = []
		networkData.value = new Map([['api-pool', {}], ['api-verify', {}], ['api-verify-66', {}], ['api-verify-138', {}]])
	...
	const onAddWorkspace = (e) => {
		...
		ipPool.value.acquireIP().then(res => {
			...
			for (const d of res.data.data) {
				ipPool.value.verifyIP(d.rip ? d.rip : d.ip).then(res => {
					ipPool.value.verifyIP66(d.rip ? d.rip : d.ip).then(res1 => {
						ipPool.value.verifyIP138(d.rip ? d.rip : d.ip).then(res2 => {
							const { ua, browser } = getUserAgent(e, phone, wechat, res.data.data.netWorkType)
							...
							const data = {
								...
								rip: d.rip,
								port: d.port,
								ipType: [res.data.data.netWorkType, res1.data.data.scenes.usage_type, res2.data.data[7] ? res2.data.data[7] : '未知'],
					...
					workspace.value.shift()
					message.error(err.response.headers.get('x-ca-error-message'))
				})
			}
		}).catch(error => {
			workspace.value.shift()
...
Mozilla/5.0 (Linux; Android 14; 2211133C Build/UKQ1.230804.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/126.0.6478.188 Mobile Safari/537.36 XWEB/1260117 MMWEBSDK/20240801 MMWEBID/5658 MicroMessenger/8.0.51.2720(0x28003339) WeChat/arm64 Weixin NetType/4G Language/zh_CN ABI/arm64